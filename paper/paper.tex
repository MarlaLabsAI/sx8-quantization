...
the reference GGUF). All measurements run on a single RTX 5060 Ti 16\,GB (SM120, 448\,GB/s) with CUDA
...
portable ALU-only decoder. S-Quant (ICML 2026) showed that per-sub-block adaptive ranges improve
quality-per-bit, a direction extended here with an exact accounting of bytes, PCA correction at the output,
and full engine integration. Quantization is occasionally observed to act as a regularizer on small
...
protocol and per-sample data are in \texttt{PROTOCOLO.md}.
