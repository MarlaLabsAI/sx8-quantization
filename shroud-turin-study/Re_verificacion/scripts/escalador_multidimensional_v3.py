"""
INDAGACION FUERTE v3: LA CRUZ CENTRAL COMO ESCALADOR MULTIDIMENSIONAL
====================================================================
Correcciones vs v2:
  - Takens D2 con minimo de 100 puntos (ventanas mas pequenas)
  - E2: ventana 300 puntos para el scan
  - E4: proyecciones 200x200 para perfiles de 200 puntos
  - E6: controles usan SU PROPIO perfil (bug corregido)
...
Tests:
  ...
  E5. D2 RADIAL DEL PERFIL: ventanas desde el centro
...
# ============================================================================
def takens_D2(profile, m=5, tau=5, subsample=800, min_points=100):
...
def takens_D2_scan(profile, m=5, tau=5, ventana=300, paso=50):
...
def firma_escalador(proj, m=5, tau=3):
    ...
    n = len(perfil)
    if n < 200:
        return {"D2_centro": float("nan"), "D2_extremos": float("nan"), "ratio": float("nan")}
    ...
    ext_vals = [v for v in [D2_e1, D2_e2] if not np.isnan(v)]
    D2_ext = float(np.mean(ext_vals)) if ext_vals else float("nan")
    return {"D2_centro": D2_c, "D2_extremos": D2_ext,
...
def main():
    ...
    scan = takens_D2_scan(profile, m=5, tau=5, ventana=300, paso=50)
    centro_scan = [s for s in scan if abs(s["pos"] - 416) < 150]
    extremos_scan = [s for s in scan if s["pos"] < 250 or s["pos"] > 830]
    c_vals = [s["D2"] for s in centro_scan if not np.isnan(s["D2"])]
    e_vals = [s["D2"] for s in extremos_scan if not np.isnan(s["D2"])]
    D2_centro = float(np.mean(c_vals)) if c_vals else float("nan")
    D2_extremos = float(np.mean(e_vals)) if e_vals else float("nan")
    ...
    corr = float("nan")
    if len(radios) > 3:
        corr = float(np.corrcoef(radios, D2s)[0, 1])
    ...
    for kind in kinds:
        ...
        if control_corrs[kind]:
            print(f"  [{kind}] corr(D2, dist_centro) = {np.mean(control_corrs[kind]):+.3f}±{np.std(control_corrs[kind]):.3f}", flush=True)
        else:
            print(f"  [{kind}] sin datos validos", flush=True)
    report["E6_controles"] = {k: {"mean": float(np.mean(v)), "std": float(np.std(v))} if v else {"mean": float("nan"), "std": float("nan")} for k, v in control_corrs.items()}

    # ============ CONCLUSION ============
    print("\n" + "=" * 70, flush=True)
    print("CONCLUSION: ¿LA CRUZ ES UN ESCALADOR MULTIDIMENSIONAL?", flush=True)
    print("=" * 70, flush=True)
    if not np.isnan(corr) and control_corrs["permutation"]:
    ...
    out_json = os.path.join(OUT, "escalador_multidimensional_v3_resultados.json")
