"""
NIVEL 2: 16 PRUEBAS SOBRE MATRIZ DE RECURRENCIA
=================================================
Aplica las mismas 16 pruebas sindonológicas sobre la matriz de recurrencia
de la imagen 3 (sepia) para comparar con Nivel 1 (imagen original)
"""
...
def check_vram(required_mb):
    ...
    if free < required_mb * 1.5:
        print(f"   [WARNING] VRAM insuficiente ({free:.0f}MB libre). Usando CPU.")
        return False
    return True
...
def box_count_single(args):
    thresh, matrix = args
    binary = (matrix > thresh).astype(np.uint8)
    sizes = [2, 4, 8, 16, 32]
    counts = []
    h, w = binary.shape
    for size in sizes:
...
def compute_entropy_row(args):
    r, half_w, matrix = args
    ws = 9
    row_entropies = []
    for c in range(half_w, matrix.shape[1] - half_w):
        patch = matrix[r-half_w:r+half_w+1, c-half_w:c+half_w+1]
        hist, _ = np.histogram(patch.flatten(), bins=16)
        hist = hist / hist.sum()
...
def generate_recurrence_matrix(img_path):
    ...
    if check_vram(500):  # Necesita ~500MB para 1080x1080
...
# ============================================================================
# 16 PRUEBAS SOBRE MATRIZ DE RECURRENCIA
# ============================================================================

def test_1_z_relief(R, name):
    ...
    if check_vram(100):
        ...
        R_4d = R_tensor.unsqueeze(0).unsqueeze(0)
        Z = F.conv2d(R_4d, kernel, padding=kernel_size//2).squeeze().cpu().numpy()
    else:
        Z = cv2.GaussianBlur(R, (5, 5), 0)
...
def test_2_fft_2d(R, name):
    ...
    if check_vram(200):
        ...
        f_transform = scipy_fft2(R.astype(float))
...
def test_3_gradient_isotropy(R, name):
    ...
    if check_vram(100):
        ...
        sobel_x = F.conv2d(R_4d, sobel_x_kernel, padding=1).squeeze()
        sobel_y = F.conv2d(R_4d, sobel_y_kernel, padding=1).squeeze()
        ...
        sobel_x = cv2.Sobel(R, cv2.CV_64F, 1, 0, ksize=3)
        sobel_y = cv2.Sobel(R, cv2.CV_64F, 0, 1, ksize=3)
...
def test_4_spatial_autocorrelation(R, name):
    print(f"   Test 4: Autocorrelación Espacial 2D ({name})")
    
    if check_vram(200):
        R_tensor = to_gpu_tensor(R)
        R_norm = R_tensor - R_tensor.mean()
        
        f_img = torch_fft2(R_norm)
        ...
        R_norm = R.astype(float) - R.mean()
        f_img = scipy_fft2(R_norm)
...
def test_5_multifractal_spectrum(R, name):
    print(f"   Test 5: Espectro Multifractal ({name})")
    
    thresholds = np.linspace(0.1, 0.9, 20)
    tasks = [(t, R) for t in thresholds]
    
    if cpu_count() > 2:
        with Pool(min(cpu_count(), 8)) as pool:
            dimensions = pool.map(box_count_single, tasks)
    else:
        dimensions = [box_count_single((t, R)) for t in thresholds]
...
def test_6_laplacian_pyramid(R, name):
...
def test_7_hurst_exponent(R, name):
    print(f"   Test 7: Exponente de Hurst ({name})")
    
    h, w = R.shape
    profile = R[h // 2, :].astype(float)
...
def test_8_radon_transform(R, name):
    ...
    for i, angle in enumerate(angles):
        rotated = ndimage.rotate(R, angle, reshape=False)
...
def test_9_tomographic_cuts(R, name):
    print(f"   Test 9: Cortes Tomográficos ({name})")
    
    if check_vram(100):
        R_tensor = to_gpu_tensor(R)
        high_depth = (R_tensor > 0.66).float()
        mid_depth = ((R_tensor >= 0.33) & (R_tensor <= 0.66)).float()
        low_depth = (R_tensor < 0.33).float()
        
        n_high = int(high_depth.sum().cpu().item())
        n_mid = int(mid_depth.sum().cpu().item())
        n_low = int(low_depth.sum().cpu().item())
    else:
        high_depth = (R > 0.66).astype(np.uint8)
        mid_depth = ((R >= 0.33) & (R <= 0.66)).astype(np.uint8)
        low_depth = (R < 0.33).astype(np.uint8)
...
def test_11_local_shannon_entropy(R, name):
    print(f"   Test 11: Entropía Local ({name})")
    
    R_small = cv2.resize(R, (200, 200))
    
    ws = 9
    half_w = ws // 2
    entropy_map = np.zeros(R_small.shape)
    
    tasks = [(r, half_w, R_small) for r in range(half_w, R_small.shape[0] - half_w)]
    
    if cpu_count() > 2:
        with Pool(min(cpu_count(), 8)) as pool:
            results = pool.map(compute_entropy_row, tasks)
        for r, row_entropies in results:
            entropy_map[r, half_w:R_small.shape[1]-half_w] = row_entropies
    else:
        for r, half_w, R_small in tasks:
            _, row_entropies = compute_entropy_row((r, half_w, R_small))
            entropy_map[r, half_w:R_small.shape[1]-half_w] = row_entropies
...
def test_12_recurrence_plot(R, name):
    print(f"   Test 12: Recurrence Plot ({name})")
    
    # La matriz YA es un recurrence plot, así que calculamos sus propiedades
...
def test_13_bilateral_symmetry(R, name):
    ...
    if check_vram(100):
        R_tensor = to_gpu_tensor(R)
        left = R_tensor[:, :center]
        right = R_tensor[:, center:]
        ...
        left = R[:, :center].astype(float)
        right = R[:, center:].astype(float)
        min_width = min(left.shape[1], right.shape[1])
        left = left[:, :min_width]
        right = right[:, :min_width]
        left_norm = (left - left.mean()) / (left.std() + 1e-10)
...
def test_14_svd_analysis(R, name):
    print(f"   Test 14: SVD ({name})")
    
    if check_vram(500):
        R_tensor = to_gpu_tensor(R)
        U, S, Vt = torch.linalg.svd(R_tensor, full_matrices=False)
        S = S.cpu().numpy()
    else:
        U, S, Vt = np.linalg.svd(R.astype(float), full_matrices=False)
...
def test_15_game_of_life(R, name):
    print(f"   Test 15: Game of Life ({name})")
    
    bin_img = (R > 0.5).astype(int)
    
    h, w = bin_img.shape
    patch = bin_img[h//2-50:h//2+50, w//2-50:w//2+50]
...
def test_16_eigenvector_centrality(R, name):
    print(f"   Test 16: Centralidad de Autovector ({name})")
    
    R_small = cv2.resize(R, (40, 40))
    h, w = R_small.shape
    n_nodes = h * w
    R_flat = R_small.flatten().astype(float)
    
    if check_vram(100):
        R_tensor = torch.from_numpy(R_flat).to(DEVICE)
        ...
        for i in range(h):
            for j in range(w):
                ...
                if j < w - 1:
                    idx_right = i * w + (j + 1)
                    similarity = 1 - torch.abs(R_tensor[idx] - R_tensor[idx_right])
                    adjacency[idx, idx_right] = similarity
                    adjacency[idx_right, idx] = similarity
                if i < h - 1:
                    idx_down = (i + 1) * w + j
                    similarity = 1 - torch.abs(R_tensor[idx] - R_tensor[idx_down])
        ...
        for i in range(h):
            for j in range(w):
                ...
                if j < w - 1:
                    idx_right = i * w + (j + 1)
                    similarity = 1 - abs(R_flat[idx] - R_flat[idx_right])
                    adjacency[idx, idx_right] = similarity
                    adjacency[idx_right, idx] = similarity
                if i < h - 1:
                    idx_down = (i + 1) * w + j
                    similarity = 1 - abs(R_flat[idx] - R_flat[idx_down])
                    adjacency[idx, idx_down] = similarity
                    adjacency[idx_down, idx] = similarity
...
def main():
    ...
    print("\n" + "="*70)
    print("NIVEL 2: ANÁLISIS DE MATRIZ DE RECURRENCIA (GPU OPTIMIZED)")
    print("="*70)
    ...
    results = {
        'matrix_type': 'recurrence_matrix',
        'source_image': 'imagen3_sepia',
        'dimensions': [R.shape[0], R.shape[1]],
        'density': float(R.mean()),
        'tests': {}
    }
    
    print("\nBLOQUE A: Proyección y Frecuencias")
    results['tests']['test_1_z_relief'] = test_1_z_relief(R, 'recurrence_matrix')
    results['tests']['test_2_fft_2d'] = test_2_fft_2d(R, 'recurrence_matrix')
    results['tests']['test_3_gradient_isotropy'] = test_3_gradient_isotropy(R, 'recurrence_matrix')
    results['tests']['test_4_spatial_autocorrelation'] = test_4_spatial_autocorrelation(R, 'recurrence_matrix')
    
    print("\nBLOQUE B: Fractal y Color")
    results['tests']['test_5_multifractal_spectrum'] = test_5_multifractal_spectrum(R, 'recurrence_matrix')
    results['tests']['test_6_laplacian_pyramid'] = test_6_laplacian_pyramid(R, 'recurrence_matrix')
    results['tests']['test_7_hurst_exponent'] = test_7_hurst_exponent(R, 'recurrence_matrix')
    
    print("\nBLOQUE C: Tomográfico y Volumétrico")
    results['tests']['test_8_radon_transform'] = test_8_radon_transform(R, 'recurrence_matrix')
    results['tests']['test_9_tomographic_cuts'] = test_9_tomographic_cuts(R, 'recurrence_matrix')
    
    print("\nBLOQUE D: Geométrico y de Ridges")
    results['tests']['test_10_topological_skeleton'] = test_10_topological_skeleton(R, 'recurrence_matrix')
    results['tests']['test_11_local_shannon_entropy'] = test_11_local_shannon_entropy(R, 'recurrence_matrix')
    results['tests']['test_12_recurrence_plot'] = test_12_recurrence_plot(R, 'recurrence_matrix')
    results['tests']['test_13_bilateral_symmetry'] = test_13_bilateral_symmetry(R, 'recurrence_matrix')
    
    print("\nBLOQUE E: Algebraico y Sistemas Complejos")
    results['tests']['test_14_svd_analysis'] = test_14_svd_analysis(R, 'recurrence_matrix')
    results['tests']['test_15_game_of_life'] = test_15_game_of_life(R, 'recurrence_matrix')
    results['tests']['test_16_eigenvector_centrality'] = test_16_eigenvector_centrality(R, 'recurrence_matrix')
    
    # Guardar resultados
    output_file = os.path.join(OUTPUT_DIR, 'sindonologia_nivel2_recurrence_matrix.json')
    ...
    print(f"\n{'='*70}")
    print("RESULTADOS NIVEL 2 GUARDADOS")
    print(f"{'='*70}")
    ...
    # Resumen
    print("RESUMEN NIVEL 2 (Matriz de Recurrencia):")
    tests = results['tests']
